...
test("billing UI uses Astryx layout primitives without hand-built layout elements", async () => {
  ...
  assert.match(source, />회의 기록<\/Text>/);
  assert.match(source, />회의 참가자 수<\/Text>/);
  assert.match(source, /durationSeconds == null[\s\S]*"제한 없음"/);
  assert.match(source, /목소리는 각 사용자가 본인 것만 등록/);
